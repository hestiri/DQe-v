...
The first two graphics in this tab (Prevalence by Location-Time) are ordinary Box plots of the `prevalence` variable, which show distribution of prevalence (i.e., patient divided by population) of the selected condition among units of locations across the selected time period. A scatter plot with jittered data points are also overlayed in the background. Box plots at any specific time unit illustrate maximum, minimum, first and third quartiles, and median for the prevalence of the selected unit condition. In the example dataset, unit of time is year. The two graphics show variations in annual prevalence of conditions G, H, and B between years 1998 and 2013. Box plots are conventionally used to visualize variability. However, to add more to these plots, users can highlight time units in which Interquartile Range (first plot) and Standard Deviation (second plot) are within a certain range. This interactive function allows users to highlight particular time units that have higher Interquartile Range or Standard Deviation than the average over the selected time period. In figure 2, for example, years in which Interquartile Range and Standard Deviation for conditions G, H, and B are between one to two and half times more than the average Interquartile Range and between two and half and six and half more than the average Standard Deviation within 1998 and 2013 are highlighted. A continous color spectrum from gold to red is dynamically assigned to the minimum and maximum of selected ranges for better visual presentation of variability.
...
Last plot, Overall Patient Population Over Time, is a scater plot of the overall population within location units (e.g., primary care clinics), regardless of condition unit. This plot illustrates the changing pattern in population seen at location units within the selected time units. For example, in Figure 2, high variability between years 2001 and 2005 can be related to the relatively smaller population size across location units.
  
### The Density Plots Tab
 
To complement the visualizations provided in **Exploratory Analysis**, the **Density Plots** tab displays smoothed probability density function of a selected variable for a selected condition and time range (Figure 3). This tab allows the users to look at changes in distribution of their variable of interest, be it `prevalence`, `patient`, or `population`. Allowing the user to change the variable of interest will provide a more wholistic preview of data distribution.

<img src="graphics/tab3-menu.png" style="height:380px;">

Figure 4. The Density Plots Tab's Menu Bar


Figure 5 illustrates annual probability distribution functions for `Condition_B` between years 2004 and 2013. The X axis in each plot represents the selected variable from the UI, which in this example is `prevalence` and the Y axis represents density. In the example illustration in Figure 5, distribution of prevalence values is for the most part skewed to the left. In 2010, the plot shows that there was a peak observed in a relatively prevalence value.


![alt text](graphics/tab3.png)
Figure 5. Visualization of probability density functions in Density Plots Tab


## Regression-based Analysis
The **Regression-based Analysis** tab uses simple predictive analytics to detect data points (i.e., single observations within a unit condition, time, and location) that are statistically above or below what would be their expacted value given data points in proceeding and preceeding time units. These data points are marked as anomaly and displayed on two visualizations. Calculation of expected values are based on a series of raw polynomial regression models that predict prevalence with time unit and population for each location units, using `poly()` in R. The regression function internally sets up the model matrix with the raw coding x, x^2, ..., x^n, when x is the variable of interest and n is the degree of the polynomial regression. 

Accordingly, the regression models are estimated using the following function:
$$ prevalence = \sum_{i=1}^n uTime^i + \sum_{j=1}^n population^j$$

where n is the degree of polynomial regression.

The application uses polynomial regression as a smoothing mechanism. That is, the users can set up the smoothing degree in the UI (maximum has been set to 4) and compare the difference in anomalies detected via a linear line and a smoother polynomial line. As illustrated in Figure 6, the regression based analysis produces three outputs: two plots and a difference table. 


![alt text](graphics/tab4.png)
Figure 6. Outputs of the Regression-based Analysis

First plot from top is a scatter plot of log of number of patients with the selected condition (i.e., patient) at each unit location and time. Dots are color coded by unit location, and similar to the scatter plot in figure 1, size of the dots represent log of total population at each clinic and time unit. On top of the prevalence scatter plot location units that have been detected as anomaly using polynomial regression estimates are highlighted. White dots with black stroke line are anomalies detected with linear regression models (where the actual prevalence value at location x and time y is statistically different from the estimated values given all data points for location x within the selected time unit range). Red dots on top of white dots show anomalies detected with polynomial regression model. The default is set to 1, so that red dots overlap the white dots. Once the user increases the polynomial degree (i.e., moves towards smoother regression lines), less red dots are expected. Second plot, which has the same x and y axes as the first plot, only highlights the location names detected as anomaly with polynomial models. The data table shows the time and location units where the outcomes of linear and polynomial models do not conform with each other. 
